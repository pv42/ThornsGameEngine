...
public class Particle {
    ...
    public Particle(ParticleTexture texture,Vector3f position, Vector3f velocity, float gravityEffect, float lifeLength, float rotation, float scale) {
        ...
        this.scale = scale;
        this.atlasOffset = texture.isRandomizeAtlas() ? (int) (Math.random() * texture.getNumberOfRows() * texture.getNumberOfRows()) : 0;
    ...
    private void updateTextureCoordsInfo() {
        ...
        setTextureOffset(texOffset1, (index1 + (int) atlasOffset) % stageCount);
        setTextureOffset(texOffset2, (index2 + (int) atlasOffset) % stageCount);

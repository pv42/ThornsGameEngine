...
void main(void) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if(CELL_SHADING_LEVELS >= 0.0) {
            float level = floor(brightness * CELL_SHADING_LEVELS);
            brightness = level / CELL_SHADING_LEVELS;
        }
        ...
        if(CELL_SHADING_LEVELS >= 0.0) {
                    float level = floor(dampedFactor * CELL_SHADING_LEVELS);
                    dampedFactor = level / CELL_SHADING_LEVELS;
        }
